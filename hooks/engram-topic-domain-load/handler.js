...
`;
  ...
  var require_errors = __commonJS2((exports2, module2) => {
    var { format: format3, inspect: inspect2 } = require_inspect(), { AggregateError: CustomAggregateError } = require_primordials(), AggregateError2 = globalThis.AggregateError || CustomAggregateError, kIsNodeError = Symbol("kIsNodeError"), kTypes = ["string", "function", "number", "object", "Function", "Object", "boolean", "bigint", "symbol"], classRegExp = /^([A-Z][a-z0-9]*)+$/, codes = {};
  ...
  var require_util = __commonJS2((exports2, module2) => {
    ...
    module2.exports = { AggregateError: AggregateError2, kEmptyObject: Object.freeze({}), once(callback) {
      ...
      return function() {};
    }, format: format3, inspect: inspect2, types: { isAsyncFunction(fn) {
...
var init_crypto = __esm(() => {
  ...
  require_der2 = __commonJS2((exports, module) => {
    ...
    DERNode.prototype._decodeObjid = function(buffer, values, relative2) {
  ...
  require_der3 = __commonJS2((exports, module) => {
    ...
    DERNode.prototype._encodeObjid = function(id, values, relative2) {
      if (typeof id === "string") {
        if (!values)
...
var {existsSync: existsSync4, readFileSync: readFileSync4} = (() => ({}));

// hooks/_lib/domain-resolve.ts
var {existsSync: existsSync2, readFileSync: readFileSync2, writeFileSync} = (() => ({}));
...
init_crypto();
var {existsSync: existsSync3, readFileSync: readFileSync3} = (() => ({}));

// hooks/_lib/system-event.ts
var {spawnSync} = (() => ({}));
function enqueueSystemEventToSession(params) {
  const sessionKey = (params.sessionKey || "").trim();
  const text = params.text || "";
  if (!sessionKey)
    return { ok: false, error: "empty sessionKey", sessionKey: "" };
  if (!text.trim())
    return { ok: false, error: "empty text", sessionKey };
...
var SYSTEM_EVENT_HASH_MARKER = /<!-- engram-system-event-hash:([a-f0-9]{8}) -->/g;
function readLatestSystemEventHash(text) {
  if (!text)
    return null;
...
function readLatestHashFromNote(notePath) {
  if (!existsSync3(notePath))
    return null;
  try {
    return readLatestSystemEventHash(readFileSync3(notePath, "utf-8"));
  } catch {
    return null;
  }
}
...
function lastChangelogEntry(p) {
  ...
  for (let i2 = 0;i2 < lines.length; i2++) {
    if (/^##\s+\d{4}-\d{2}-\d{2}/.test(lines[i2]))
      entryStarts.push(i2);
  }
...
var handler = async (event) => {
  ...
  if (!result.ok) {
    console.warn(`[engram-topic-domain-load] system-event injection failed for "${domainName}" (chat=${absChatId}, topic=${topicId}): ${result.error}; next message will retry`);
    return;
  }
  console.log(`[engram-topic-domain-load] Injected domain context + agents for "${domainName}" → chat ${absChatId}/topic ${topicId} via system-event (hash ${contentHash}, agents ${agents.source}, ${result.bytesSent} bytes)`);
};
