...
3. **Delivery channel**: `openclaw system event --mode now --session-key <agent:sample-agent:telegram:group:<chatId>:topic:<topicId>> --text <9146-char payload> --timeout 10000`. The gateway enqueues the system event for delivery in the current/next agent iteration. **v3.5 changed this from `writeFileSync` blocks in the daily note** (write-then-hope anti-pattern, see `memory/tmp/engram-v35-addendum.md` §2).
4. **Idempotency**: 
