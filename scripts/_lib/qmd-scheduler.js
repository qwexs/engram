...
export function buildQmdMaintenancePayload({ workspace, manifest, report, timeoutMs = 600000 }) {
  ...
  const argv = ["bun", join(workspace, "skills/engram/scripts/qmd-maintenance-coordinator.ts"),
    "--manifest", manifest, "--workspace", workspace, "--timeout-ms", String(timeoutMs)];
  const execArgs = { command: argv.map(shellQuote).join(" ") + " > " + shellQuote(report),
...
export function inspectQmdMaintenancePayload(payload) {
  ...
  try {
    ...
    if (!execArgs || typeof execArgs.command !== "string" || !isAbsolute(execArgs.workdir ?? "") || !isAbsolute(report ?? "")) return null;
