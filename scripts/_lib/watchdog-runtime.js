...
export function collectWatchdogRuntime(workspace, { plugin = true } = {}) {
  ...
  let config = null;
  const configPath = process.env.OPENCLAW_CONFIG_PATH || join(process.env.OPENCLAW_STATE_DIR || join(homedir(), ".openclaw"), "openclaw.json");
...
export function auditHeartbeatScheduler(engram, inventory) {
  ...
  const jobs = inventory.jobs.filter(job => job.name === name);
  ...
  for (const job of jobs) {
    ...
    const deterministic = job.payload?.kind === "command"
      ...
      : payload.includes("Generated by install-deterministic-heartbeat-cron.js");
