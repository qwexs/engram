...
const DEPLOYMENT_SOURCE_FILES = [
  ...
  "scripts/oll-rule-notifications.ts",
  ...
  "src/oll/memory-candidate-store-v2.ts",
  "src/oll/memory-candidate-runtime-v2.ts",
...
function buildScript(options: { workspace: string; stateRoot: string; registrySnapshot: string; allowedRoots: string[]; declarationPath: string; declaration: JsonObject }): string {
  ...
  for (let index = 0; index < argv.length; index += 1) if (argv[index] === "--workspace") workspacePaths.push(String(argv[index + 1]));
  ...
  const notificationPendingCommand = [
    "bun", "./skills/engram/scripts/oll-rule-notifications.ts", "pending",
    ...workspacePaths.flatMap((workspace) => ["--workspace", workspace]),
  ].map(q).join(" ");
  const notificationAckPrefix = ["bun", "./skills/engram/scripts/oll-rule-notifications.ts", "ack"].map(q).join(" ");
...
for (const delivery of notificationBatch.deliveries ?? []) {
  ...
  const ack = ${q(notificationAckPrefix)} + " --workspace " + JSON.stringify(delivery.workspace)
    + " --notification-id " + JSON.stringify(delivery.notificationId)
    + " --message-id-uri " + JSON.stringify(encodeURIComponent(messageId));
  await execText(ack, 30000, remaining(120));
  notificationsDelivered += 1;
}
return { state: { lastRunAt: new Date().toISOString(), runtimeSourceRevision: RUNTIME_SOURCE_REVISION, reconciliationWorkspaces: reconciliation.workspaces?.length ?? 0, dispatches, notificationsDelivered, batchStatus: step.report?.status ?? null, maxConcurrentRethinkRuns: step.report?.maxConcurrentRethinkRuns ?? 0 } };`;
}
...
function candidate() {
  ...
  const payload = { kind: "script", script, timeoutSeconds: OPENCLAW_SCRIPT_TIMEOUT_SECONDS, toolBudget: 120, toolsAllow: ["exec", "process", "sessions_spawn", "message"] };
