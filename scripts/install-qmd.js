#!/usr/bin/env bun
// memory-system/scripts/install-qmd.js
// Install QMD (local or Jina fork) with interactive variant selection
// Usage: bun skills/memory-system/scripts/install-qmd.js [--variant local|jina] [--jina-key <key>] [--help]
...
Usage:
  bun skills/memory-system/scripts/install-qmd.js [options]
...
Examples:
  ...
  bun skills/memory-system/scripts/install-qmd.js --variant jina --jina-key jina_xxx
  bun skills/memory-system/scripts/install-qmd.js -v local
`);
  process.exit(0);
}
...
if (variant === 'local') {
  ...
  if (!jinaKey) {
    console.log('\n🔑 Jina AI API key required (free at https://jina.ai/api-key)');
...
if (qmdInstalled()) {
  ...
  console.log(`\nNext: bun skills/memory-system/scripts/init.js`);
} else {
  console.log('  ⚠️  QMD not found in PATH. You may need to restart your terminal.');
  console.log('  Then run: bun skills/memory-system/scripts/init.js');
}
