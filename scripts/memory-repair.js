...
// Use ONLY for correcting schema-level field issues (e.g. `confidence: null` written
// before a fix). Do NOT use this to add or change factual content — that goes
// through `memory-write.js`.
//
// Examples:
...
if (opts.help || opts.h) {
  console.log([
    "memory-repair.js",
    "",
    "Schema repair for an existing KG fact. Corrects field values (e.g. confidence)",
    ...
    "",
    "Usage:",
    "  bun skills/engram/scripts/memory-repair.js --entity <path> --id <fact-id> --confidence <0-1>",
...
if (!entity || !factId || newConfidenceRaw === undefined) {
  console.error("❌ Требуются --entity, --id, --confidence");
  process.exit(1);
}

const newConfidence = Number(newConfidenceRaw);
...
if (beforeConfidence === newConfidence) {
  console.log(JSON.stringify({ status: "noop", entity, id: factId, confidence: newConfidence }));
  process.exit(0);
}
...
const result = {
  ...
  id: factId,
  path: entityPath,
  before: { confidence: beforeConfidence },
...
facts[idx] = { ...before, confidence: newConfidence };
await Bun.write(entityPath, JSON.stringify(payload, null, 2) + "\n");

if (opts.validate) {
  try {
    const proc = Bun.spawn(["bun", join(import.meta.dir, "validate.js"), "--agent-id", "main"], {
      cwd: WORKSPACE,
      stdout: "pipe",
      stderr: "pipe",
    });
