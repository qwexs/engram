...
//   --tension "описание противоречия" --fact1 "id1" --fact2 "id2"
//   [--type factual|temporal|priority] [--description "контекст"] [--dry-run]
...
const tensionType = opts.type || "factual";
if (!VALID_TYPES.includes(tensionType)) {
  console.error(`❌ --type должен быть: ${VALID_TYPES.join(", ")}`);
  process.exit(1);
}

// confidence: 0.0–1.0, default null (unknown / manual)
const confidenceRaw = opts.confidence !== undefined ? parseFloat(opts.confidence) : null;
const confidence = (confidenceRaw !== null && !isNaN(confidenceRaw))
  ? Math.min(1, Math.max(0, parseFloat(confidenceRaw.toFixed(2))))
  : null;

// --- Поиск факта в KG: возвращает { found: bool, text: string|null } ---
async function findFact(factId) {
  ...
  for await (const file of glob.scan({ cwd: lifeDir, absolute: true })) {
    try {
      ...
      if (fact) return { found: true, text: fact.fact || null };
    } catch {}
  }
  return { found: false, text: null };
}

const fact1Info = await findFact(opts.fact1);
if (!fact1Info.found) {
  console.error(`❌ Факт ${opts.fact1} не найден в KG (life/**/items.json)`);
  process.exit(1);
}
const fact2Info = await findFact(opts.fact2);
if (!fact2Info.found) {
...
if (existing.tensions.length > 0) {
  ...
  for (const tensionId of existing.tensions.slice(-20)) {
    try {
      ...
      if (tData.status === "resolved" || tData.status === "dissolved") continue;
...
const newTension = {
  ...
  confidence,
  fact1: opts.fact1,
  fact1Text: fact1Info.text ? fact1Info.text.slice(0, 200) : null,
  fact2: opts.fact2,
  fact2Text: fact2Info.text ? fact2Info.text.slice(0, 200) : null,
...
if (!newTension.fact1Text) delete newTension.fact1Text;
if (!newTension.fact2Text) delete newTension.fact2Text;
...
if (opts["dry-run"]) {
  console.log(JSON.stringify({
    ...
    confidence,
    fact1: opts.fact1,
    fact1Text: fact1Info.text?.slice(0, 100),
    fact2: opts.fact2,
    fact2Text: fact2Info.text?.slice(0, 100),
    would_write: {
