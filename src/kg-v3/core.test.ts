...
describe("KG v3 normative schema and authority", () => {
  test("accepts positive fixture and rejects negative fixture", () => {
  ...
  for (const mode of ["absent", "legacy-contained"] as const) {
    test(`${mode} authority denies v3 writes`, () => {
      const { core, caller } = fixture(mode);
      expect(core.write(request(mode), caller).reason).toBe("CALLER_NOT_AUTHORIZED");
    });
  }

  for (const mode of ["canary", "enabled"] as const) {
    test(`${mode} authority allows only capability-authorized caller`, () => {
      const { core, caller } = fixture(mode);
      const denied = { ...caller, capabilities: [] } as TrustedKgCallerContext;
      expect(core.write(request(`${mode}-denied`), denied).reason).toBe("CALLER_NOT_AUTHORIZED");
      expect(core.write(request(`${mode}-allowed`), caller).status).toBe("committed");
    });
  }
});

describe("KG v3 deterministic writer", () => {
  test("derived operation id is stable and has no runtime timestamp input", () => {
  ...
  test("same operation returns byte-equivalent receipt; changed payload conflicts", () => {
    const { core, caller } = fixture();
    const input = request("same-op");
    const first = core.write(input, caller);
    expect(core.write(input, caller)).toEqual(first);
    const changed = structuredClone(input);
    changed.assertion.object.value = "different";
    expect(core.write(changed, caller).reason).toBe("OPERATION_CONFLICT");
    expect(readdirSync(join(core.assertionsRoot))).toHaveLength(1);
  });

  test("arbitrary sha256 operation id fails canonical provenance closed", () => {
    const { core, caller } = fixture();
    const input = request("non-canonical-operation");
    input.assertion.provenance.operationId = op("arbitrary-but-valid");
    expect(core.write(input, caller)).toMatchObject({ status: "rejected", reason: "PROVENANCE_MISSING" });
    expect(existsSync(core.operationsRoot)).toBe(false);
  });

  test("exact semantic duplicate skips without creating a second assertion", () => {
    const { core, caller } = fixture();
    const first = core.write(request("duplicate-1"), caller);
    const duplicate = core.write(request("duplicate-2"), caller);
  ...
  for (const point of ["after-prepared", "after-assertion-store", "after-previous-store", "after-store-committed", "after-committed"] as KgCrashPoint[]) {
    test(`recovers ${point} with no duplicate`, () => {
      ...
      expect(() => crashing.write(input, caller)).toThrow(`simulated-crash:${point}`);
      const recovered = new KgV3Core({ workspace: root, workspaceId: "main" });
      const receipt = recovered.write(input, caller);
      expect(receipt.status).toBe("committed");
      expect(recovered.write(input, caller)).toEqual(receipt);
      expect(readdirSync(recovered.assertionsRoot)).toHaveLength(1);
      expect(recovered.current()).toHaveLength(1);
    });
  }

  test("a different next operation first recovers an interrupted entity WAL", () => {
    const { root, caller } = fixture();
    const interrupted = request("interrupted-before-next");
    expect(() => new KgV3Core({ workspace: root, workspaceId: "main", crashAt: "after-prepared" }).write(interrupted, caller)).toThrow();
    const core = new KgV3Core({ workspace: root, workspaceId: "main" });
    const next = core.write(request("different-next-operation"), caller);
    expect(next).toMatchObject({ status: "skipped", reason: "DUPLICATE" });
    expect(core.current()).toHaveLength(1);
    expect(readdirSync(core.assertionsRoot)).toHaveLength(1);
  });

  test("replacement supersedes prior assertion and return-to-old creates a new assertion", () => {
    const { core, caller } = fixture();
    const first = core.write(request("replace-1", "A"), caller);
    const second = core.write(request("replace-2", "B", first.assertionId), caller);
    const third = core.write(request("replace-3", "A", second.assertionId), caller);
    expect(new Set([first.assertionId, second.assertionId, third.assertionId]).size).toBe(3);
    expect(core.current()).toMatchObject([{ id: third.assertionId, object: { value: "A" } }]);
  ...
  test("retraction removes current assertion without deleting its body", () => {
    const { core, caller } = fixture();
    const created = core.write(request("retract-create"), caller);
  ...
  test("tampered retraction action provenance fails journal recovery closed", () => {
    const { core, caller } = fixture();
    const created = core.write(request("tamper-retract-create"), caller);
    ...
    expect(() => core.recover()).toThrow("invalid KG v3 operation journal");
  });
});

describe("KG v3 fail-closed boundaries", () => {
  test("cross-workspace, cross-scope, unresolved entity and wrong store fail closed", () => {
    const { core, caller } = fixture();
    const crossWorkspace = request("cross-workspace");
    crossWorkspace.assertion.workspaceId = "other";
    expect(core.write(crossWorkspace, caller).reason).toBe("WORKSPACE_MISMATCH");
    const crossScope = request("cross-scope");
    crossScope.assertion.scope = ["other-project"];
    expect(core.write(crossScope, caller).reason).toBe("WRONG_STORE");
    ...
    expect(core.write(unresolved, caller).reason).toBe("ENTITY_UNRESOLVED");
    const progress = request("progress");
    progress.intent.statementClass = "progress";
    expect(core.write(progress, caller).reason).toBe("WRONG_STORE");
  });

  test("concurrent replacements have exactly one winner", async () => {
    const { root, core, caller } = fixture();
    const initial = core.write(request("concurrent-initial", "A"), caller);
    ...
    expect(new KgV3Core({ workspace: root, workspaceId: "main" }).current()).toHaveLength(1);
  });
  ...
  test("current reader recovers and never exposes an after-store partial commit", () => {
    const { root, caller } = fixture();
    const input = request("reader-barrier");
    expect(() => new KgV3Core({ workspace: root, workspaceId: "main", crashAt: "after-assertion-store" }).write(input, caller)).toThrow();
    expect(existsSync(join(root, "life", "v3", "current-summary.md"))).toBe(false);
    const current = new KgV3Reader({ workspace: root, workspaceId: "main" }).current();
  ...
  test("v2 adapter is explicit and never mixed into current", () => {
    const { root } = fixture();
