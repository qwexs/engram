...
describe("PR3 default-context safety", () => {
  ...
  test("bootstrap hook injects only authorized main current projection and no-ops otherwise", async () => {
    ...
    const bootstrapFiles = () => [{ name: "MEMORY.md", path: join(workspace, "MEMORY.md"), content: "stale writer guidance", missing: false }];
    const event = { type: "agent", action: "bootstrap", context: { workspaceDir: workspace, sessionKey: "agent:main:main", bootstrapFiles: bootstrapFiles() }, messages: [] as string[] };
    ...
    expect(event.context.bootstrapFiles[1]).toMatchObject({ name: "BOOTSTRAP.md", path: join(workspace, "life", "v3", "current-summary.md"), missing: false });
    expect(event.context.bootstrapFiles[1].content).toContain("engram-kg-v3-current");
    expect(event.context.bootstrapFiles[1].content).toContain("engram.kg-context.bootstrap-file.v1");
    const bootstrapOnly = { type: "agent", action: "bootstrap", context: { workspaceDir: workspace, sessionKey: "agent:main:main", bootstrapFiles: bootstrapFiles() } };
    await kgContextHook(bootstrapOnly);
    expect(bootstrapOnly.context.bootstrapFiles).toHaveLength(2);
    expect(bootstrapOnly.context.bootstrapFiles[1].content).toContain("engram.kg-context.bootstrap-file.v1");
    const messageOnly = { type: "agent", action: "bootstrap", context: { workspaceDir: workspace, sessionKey: "agent:main:main" }, messages: [] as string[] };
    await kgContextHook(messageOnly);
    expect(messageOnly.messages).toHaveLength(1);
    ...
    const schemaMismatch = { ...event, context: { ...event.context, bootstrapFiles: bootstrapFiles() }, messages: [] as string[] };
    ...
    expect(schemaMismatch.context.bootstrapFiles).toHaveLength(1);
    ...
    const releaseMismatch = { ...event, context: { ...event.context, bootstrapFiles: bootstrapFiles() }, messages: [] as string[] };
    await kgContextHook(releaseMismatch);
    expect(releaseMismatch.messages).toEqual([]);
    expect(releaseMismatch.context.bootstrapFiles).toHaveLength(1);
    wrongRelease.releaseDigest = op("release");
    json(join(workspace, "memory-state", "kg-v3", "default-context.json"), wrongRelease);
    const direct = { ...event, context: { ...event.context, bootstrapFiles: bootstrapFiles(), sessionKey: "telegram-direct-1001", trustedActorContext: { trusted: true, contextKind: "direct", actorId: "1001" } }, messages: [] as string[] };
    ...
    expect(direct.context.bootstrapFiles).toHaveLength(2);
    const canonicalDirect = { ...event, context: { ...event.context, bootstrapFiles: bootstrapFiles(), sessionKey: "agent:main:telegram:direct:1001", trustedActorContext: { trusted: true, contextKind: "direct", actorId: "1001" } }, messages: [] as string[] };
    ...
    json(join(workspace, "memory-state", "kg-v3", "authority.json"), authority);
    const colonGrantDirect = { ...event, context: { ...event.context, bootstrapFiles: bootstrapFiles(), sessionKey: "agent:main:telegram:direct:1003" }, messages: [] as string[] };
    await kgContextHook(colonGrantDirect);
    expect(colonGrantDirect.messages).toHaveLength(1);
    const primaryGrantDirect = { ...event, context: { ...event.context, bootstrapFiles: bootstrapFiles(), sessionKey: "agent:main:telegram:direct:1002" }, messages: [] as string[] };
    await kgContextHook(primaryGrantDirect);
    expect(primaryGrantDirect.messages).toHaveLength(1);
    const mismatchedActor = { ...event, context: { ...event.context, bootstrapFiles: bootstrapFiles(), sessionKey: "agent:main:telegram:direct:1002", trustedActorContext: { trusted: true, contextKind: "direct", actorId: "1003" } }, messages: [] as string[] };
    await kgContextHook(mismatchedActor);
    expect(mismatchedActor.messages).toEqual([]);
    expect(mismatchedActor.context.bootstrapFiles).toHaveLength(1);
    const untrustedDirect = { ...event, context: { ...event.context, bootstrapFiles: bootstrapFiles(), sessionKey: "agent:main:telegram:direct:1002", trustedActorContext: { trusted: false, contextKind: "direct", actorId: "1002" } }, messages: [] as string[] };
    await kgContextHook(untrustedDirect);
    expect(untrustedDirect.messages).toEqual([]);
    expect(untrustedDirect.context.bootstrapFiles).toHaveLength(1);
    const topic = { ...event, context: { ...event.context, bootstrapFiles: bootstrapFiles(), sessionKey: "telegram-group-1-topic-2" }, messages: [] as string[] };
    await kgContextHook(topic);
    expect(topic.messages).toEqual([]);
    expect(topic.context.bootstrapFiles).toHaveLength(1);
    writeFileSync(join(workspace, "life", "v3", "current-summary.md"), "legacy items.json\n");
    const leaked = { ...event, context: { ...event.context, bootstrapFiles: bootstrapFiles() }, messages: [] as string[] }; await kgContextHook(leaked); expect(leaked.messages).toEqual([]); expect(leaked.context.bootstrapFiles).toHaveLength(1);
  });
});
