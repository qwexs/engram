...
describe("PR3 default-context safety", () => {
  ...
  test("bootstrap hook injects only authorized main current projection and no-ops otherwise", async () => {
    ...
    json(join(workspace, "memory-state", "kg-v3", "default-context.json"), wrongRelease);
    const direct = { ...event, context: { ...event.context, sessionKey: "telegram-direct-1001", trustedActorContext: { trusted: true, contextKind: "direct", actorId: "1001" } }, messages: [] as string[] };
    ...
    const untrustedDirect = { ...event, context: { ...event.context, sessionKey: "agent:main:telegram:direct:1002", trustedActorContext: { trusted: false, contextKind: "direct", actorId: "1002" } }, messages: [] as string[] };
