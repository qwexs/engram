...
export function fixture(texts=['I can commit the timer fix.','Yes, do that.'],actors=['actor-a','actor-a'],outcomes?:string[],contexts?:any[],episodes?:any[]):CompiledBatchBundleV1 {
 ...
 const sources=texts.map((text,i)=>{
  ...
  const payload={...(episodes?.[i]?{episodeContext:episodes[i]}:{}),...(contexts?.[i]?{replyContext:contexts[i]}:{}),source:{role:'user',text,actorId:actors[i]},outcome:{role:'assistant',text:outcomes?.[i]??(i===0?'I can commit the timer fix.':'Timer fix committed.')}};
  ...
  return {envelope:{schema:'engram.memory-observation-job.v1',traceId,sourceTurnId,scope,sourceCompletedAt,sourceDigest:deriveSourceDigest(sourceTurnId,scope as any,sourceCompletedAt),evidenceDigest:sha256({schema:evidence.schema,traceId,scope,payload} as JsonValue),policyVersion:'fixture-v1',policyDigest,evidenceRefs:[{kind:'source-turn',ref:sourceTurnId,digest:sha256(text)},...[...(contexts?.[i]?.pairs??[]),...(episodes?.[i]?.pairs??[])].map((p:any)=>({kind:'message',ref:scope.runtimeSessionKey+'#'+p.transportMessageId,digest:p.evidenceDigest}))],authority:{id:'openclaw-runtime',version:'v1',digest:sha256('runtime')},admittedAt:sourceCompletedAt},evidence};
 });
