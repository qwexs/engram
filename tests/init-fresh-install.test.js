...
describe("init.js — fresh install happy path", () => {
  ...
  test("init creates template files", async () => {
    ...
    expect(config.qmd.collections).toEqual(["openclaw-memory-agent-main-main"]);
    expect(config.qmd.maintenance).toEqual({ mode: "coordinated" });
...
`);
    ...
    expect(added.args).toContain("--disabled");
  ...
  test("init --workspace-only skips shared hooks, config edits, cron, and gateway restart", async () => {
    const fake = join(workspace, "fake-openclaw.js");
    const log = join(workspace, "fake-openclaw-log.txt");
    writeFileSync(fake, `#!/usr/bin/env bun
...
`);
    ...
    const result = await runInit(workspace, ["--workspace-only"], {
      skipGatewayRestart: false,
      extraEnv: { ENGRAM_SKIP_HOOK_INSTALL: "0", ENGRAM_OPENCLAW: fake },
    });
    expect(result.exitCode, result.stderr || result.stdout).toBe(0);
    expect(existsSync(join(workspace, "engram.json"))).toBeTrue();
    expect(existsSync(log)).toBeFalse();
